...
static void before_kexec(int luo_fd)
{
	...
	create_state_file(luo_fd, "state-session", STATE_TOKEN, /*next_stage=*/2);

	session_fd = luo_create_session(luo_fd, "device-session");
...
static void after_kexec(int luo_fd, int state_session_fd)
{
	...
	session_fd = luo_retrieve_session(luo_fd, "device-session");
	VFIO_ASSERT_GE(session_fd, 0);
...
int main(int argc, char *argv[])
{
	...
	return luo_test(argc, argv, "state-session", before_kexec, after_kexec);
}
