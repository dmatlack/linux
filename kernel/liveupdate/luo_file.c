...
static int luo_alloc_files_mem(struct luo_session *session)
{
	size_t size;
	void *mem;
...
static void luo_free_files_mem(struct luo_session *session)
{
...
int luo_preserve_file(struct luo_session *session, u64 token, int fd)
{
	...
	err = luo_alloc_files_mem(session);
	if (err)
		goto  err_files_mem;
